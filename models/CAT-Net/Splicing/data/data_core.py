...
class SplicingDataset(Dataset):
    def __init__(self, crop_size, grid_crop, blocks=('RGB',), mode="train", DCT_channels=3, read_from_jpeg=False, class_weight=None, user_data_root=None):
        self.dataset_list = []
        if mode == "train":
            # Use user's dataset
            if user_data_root:
                self.dataset_list.append(UserDataset(crop_size, grid_crop, blocks, DCT_channels, user_data_root, "train", read_from_jpeg=read_from_jpeg))
            else:
                # Original datasets (commented out)
            ...
            # Use user's dataset for validation
            if user_data_root:
                self.dataset_list.append(UserDataset(crop_size, grid_crop, blocks, DCT_channels, user_data_root, "test", read_from_jpeg=read_from_jpeg))
            else:
                # Original datasets (commented out)
    ...
    def __len__(self):
        if self.mode == 'train':
            # For user's dataset, just return the actual length
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            # For user's dataset, use the same approach as validation
            it = 0
            while True:
                if index >= len(self.dataset_list[it]):
                    index -= len(self.dataset_list[it])
