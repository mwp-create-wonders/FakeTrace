...
class MLPBlock(nn.Sequential):
    ...
    MLPBlock
        实现了 Transformer 中的 MLP 模块，包含两层线性层和 GELU 激活函数。
        使用 Xavier 初始化和正态分布初始化偏置项。
    """
...
class EncoderBlock(nn.Module):
    ...
    实现了 Transformer 编码器块，包含：
        自注意力模块（MultiheadAttention）。
        两个 LayerNorm 归一化层。
        MLP 模块。
    支持残差连接和 Dropout。
    """
    ...
    def __init__(
        ...
        # Attention block
        self.ln_1 = norm_layer(hidden_dim) # 归一化
        ...
        # 这种结构的设计使得 Transformer 编码器块能够有效地处理长序列数据，并在自然语言处理（NLP）和计算机视觉（CV）任务中表现出色。
...
class Encoder(nn.Module):
    ...
    实现了 Transformer 编码器，包含多个编码器块。
    添加了位置嵌入，并在编码器前应用 Dropout。
    最终通过 LayerNorm 输出特征。
    """
    ...
    def __init__(
        ...
        # 这种编码器结构广泛应用于自然语言处理任务，例如机器翻译、文本分类、文本生成等。通过堆叠多个编码器块，Transformer 能够捕捉输入序列中的长距离依赖关系，并生成丰富的上下文表示。
...
class VisionTransformer(nn.Module):
    ...
    实现了完整的 Vision Transformer 模型，包含：
        Patch 嵌入（通过卷积实现）。
        类别标记（Class Token）。
        Transformer 编码器。
        分类头（支持可选的表示层）。
    支持自定义卷积 stem（如用于更复杂的 Patch 嵌入）。
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self._process_input(x) # patch嵌入
        n = x.shape[0]

        # Expand the class token to the full batch
        batch_class_token = self.class_token.expand(n, -1, -1)
        x = torch.cat([batch_class_token, x], dim=1) # 添加类别标记
        ...
        x = self.heads(x) # 分类头，I checked that for all pretrained ViT, this is just a fc 
        out['logits'] = x
...
def interpolate_embeddings(
    ...
    """
    # 提供了位置嵌入的插值功能，用于调整模型的输入分辨率。支持自定义插值模式（如双三次插值）。
