...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--real_num', type=int, default=0,
                            help='max number of real images, 0 means all')
        parser.add_argument('--fake_num', type=int, default=0,
    ...
    def parse(self, print_options=True):
        ...
        opt.real_dirs = [s.strip() for s in opt.real_dirs.split(',') if s.strip()]
        opt.fake_dirs = [s.strip() for s in opt.fake_dirs.split(',') if s.strip()]

        if hasattr(opt, 'val_real_dirs') and isinstance(opt.val_real_dirs, str):
            opt.val_real_dirs = [s.strip() for s in opt.val_real_dirs.split(',') if s.strip()]
        
        if hasattr(opt, 'val_fake_dirs') and isinstance(opt.val_fake_dirs, str):
            opt.val_fake_dirs = [s.strip() for s in opt.val_fake_dirs.split(',') if s.strip()]
